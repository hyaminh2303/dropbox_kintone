...
export default class RecordDetail extends Component {
  constructor(props) {
    ...
    this.onOpenDialogEditNameFolder =
      this.onOpenDialogEditNameFolder.bind(this);
  ...
  async validateDropboxAccessToken() {
    const {
      config: { accessToken, refreshToken, dropboxAppKey },
    } = this.props;
  ...
  async getFolderRoot() {
    ...
    const response =
      await this.findOrCreateDropboxConfigurationRecordAndGetRootPath();
  ...
  async findOrCreateDropboxConfigurationRecordAndGetRootPath() {
    const {
      config,
      config: { dropbox_configuration_app_id },
      event: { record },
    } = this.props;

    const rootConfigurationRecord = await getRootConfigurationRecord(
      dropbox_configuration_app_id
    );
    if (
      !!rootConfigurationRecord &&
      rootConfigurationRecord["errorCode"] == "invalidConfigurationAppId"
    ) {
    ...
    if (!rootConfigurationRecord) {
      showNotificationError(
        "Missing Root Configuration, please visit plugin settings to update it!"
      );
      return {
        errorCode:
          "Missing Root Configuration, please visit plugin settings to update it!",
    ...
    if (!rootConfigurationRecord.dropbox_folder_id.value) {
      ...
      metadataResponse = await this.dbx
        .filesGetMetadata({
    ...
    if (metadataResponse["errorCode"] == "notFoundFolderOnDropbox") {
      showNotificationError(
        "Not found root dropbox folder, please create a new one and update your plugin settings"
      );
      return {
    ...
    const configurationRecord = await getConfigurationRecord(
      dropbox_configuration_app_id,
      record["$id"].value
    );
    ...
    if (
      !!configurationRecord &&
      configurationRecord["errorCode"] == "invalidConfigurationAppId"
    ) {
      alert("Please endter configuration app id in plugin setting!");
      return {
        errorCode: "invalidConfigurationAppId",
    ...
    if (!!configurationRecord) {
      ...
      const recordFolderMetadataResponse = await this.dbx
        .filesGetMetadata({
      ...
      if (
        recordFolderMetadataResponse["errorCode"] == "notFoundFolderOnDropbox"
      ) {
        ...
        const createFolderResponse = await this.requestDropbox(
          "filesCreateFolderV2",
          {
            path: rootPath,
            autorename: true,
        ...
        await updateConfigurationRecord(
          ...
          {
            dropbox_folder_name: {
              value: createFolderResponse.result.metadata.name,
            },
            dropbox_folder_id: {
              value: createFolderResponse.result.metadata.id,
            },
            root_folder_name: {
              value: rootConfigurationRecord["root_folder_name"].value,
            },
            namespace_id: {
              value: rootConfigurationRecord["namespace_id"].value,
            },
            namespace_name: {
              value: rootConfigurationRecord["namespace_name"].value,
            },
          }
        );
        ...
        recordFolderMetadataResponse.result.name !=
        configurationRecord["dropbox_folder_name"].value
      ) {
        ...
        updateConfigurationRecord(
          ...
          {
            dropbox_folder_name: {
              value: recordFolderMetadataResponse.result.name,
            },
            dropbox_folder_id: {
              value: recordFolderMetadataResponse.result.id,
            },
            root_folder_name: {
              value: rootConfigurationRecord["root_folder_name"].value,
            },
      ...
      const folderName = `${record[config.selectedField].value}[${
        record["$id"].value
      }]`;
      ...
      await addChildFolderRecord(dropbox_configuration_app_id, {
        root_folder_name: {
          value: rootConfigurationRecord["root_folder_name"].value,
        },
  ...
  async requestDropbox(
    dbxMethod: string,
    args: any,
    successCallback = (error: any) => {},
  ...
  onDeleteFile(dropboxEntry) {
    showConfirm(() => {
      this.requestDropbox(
        ...
        (dbxResponse) => {
          showNotificationSuccess(
            `${dropboxEntry[".tag"]} has been deleted successfully`
          );
  ...
  uploadFile(file) {
    this.requestDropbox(
      "filesUpload",
      { contents: file, path: `${this.state.currentPathLower}/${file.name}` },
      (dbxResponse) => {
  ...
  createChildFolder(name: string) {
    ...
    this.requestDropbox(
      ...
      (error) => {
        showNotificationError(
          "Error while creating folder, please double check the folder name"
        );
      }
    );
  }
  ...
  editChildFolderName(dropboxEntry: any, newName: string) {
    ...
    if (dropboxEntry.name !== newName) {
      this.requestDropbox(
        ...
        (error) => {
          showNotificationError(
            "Error while renaming folder, please double check the folder name"
          );
        }
      );
    }
  ...
  render() {
    ...
    const {
      ...
      isDialogUploadVisible,
      isDialogFolderFormVisible,
      previewPath,
          ...
          <div className="dropbox-detail-border">
            ...
            <div className="btn-menu-wrapper">
              <Button
                ...
                color="primary"
                startIcon={
                  <FontAwesomeIcon icon={faUpload} className="fa btn-icon" />
                }
              ...
              <Button
                ...
                color="primary"
                startIcon={
                  <FontAwesomeIcon icon={faPlus} className="fa btn-icon" />
                }
            ...
            <div className="dropbox-detail-border">
              {dropboxEntries.map((dropboxEntry, index) => {
                ...
                return (
                  <div className="dropbox-item-wrapper" key={index}>
                    <div
                      className="dropbox-item-icon"
                      onClick={() => this.onClickDropboxFolder(dropboxEntry)}
                    >
                      {dropboxEntry[".tag"] === "folder" ? (
                        <FontAwesomeIcon
                          icon={faFolder}
                          className="fa folder-icon"
                        />
                      ) : (
                        <FileIcon
                          extension={`"${typeOfFile}"`}
                          {...defaultStyles[typeOfFile]}
                          color="#2ECC71"
                          glyphColor="#fff"
                        />
                      )}
                    ...
                    <div
                      className="dropbox-item-name"
                      onClick={() => this.onClickDropboxFolder(dropboxEntry)}
                    >
                      <span>{dropboxEntry.name}</span>
                    </div>
                    ...
                    <div className="dropbox-item-actions">
                      ...
                      {dropboxEntry[".tag"] == "folder" && (
                        <Button
                          ...
                          onClick={() =>
                            this.onOpenDialogEditNameFolder(dropboxEntry)
                          }
                        >
                          Rename
                        </Button>
